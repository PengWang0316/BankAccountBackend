...
const logger = helper.getLogger('BlockListener');

const startBlockListener = async function (channelName, username, orgName, websocketServer) {
  ...
  try {
    ...
    if (!channel) {
      const message = util.format('##### startBlockListener - Channel %s was not defined in the connection profile', channelName);
